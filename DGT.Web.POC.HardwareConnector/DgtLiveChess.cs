...
namespace DGT.Web.POC.HardwareConnector
{
    ...
    public sealed class DgtLiveChess : IDgtLiveChess
    {
        ...
        private const int BOARD_POLL_RETRY_DELAY = 500;
        ...
        private static async Task Send(ClientWebSocket socket, string data)
        {
            ...
            await socket.SendAsync(new ArraySegment<byte>(Encoding.UTF8.GetBytes(data)), WebSocketMessageType.Text, true, CancellationToken.None).ConfigureAwait(false);
        }
